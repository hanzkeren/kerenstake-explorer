<template>
  <div>
    <!-- Hidden featured validator section unless configured -->
    <b-card v-if="pingVals && pingVals.length > 0" title="Featured Validators" class="overflow-auto">
      <b-table
        :items="pingVals"
...
export default {
  ...
  computed: {
    // Return empty by default; can be enabled by setting window.FEATURED_VALIDATOR_IDENTITIES = ['identity1', ...]
    pingVals() {
      const ids = (window.FEATURED_VALIDATOR_IDENTITIES || [])
      if (!Array.isArray(ids) || ids.length === 0) return []
      return this.list.filter(x => ids.includes(x.description.identity))
    },
    list() {
